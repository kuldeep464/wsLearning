import axios from 'axios'
import React, { useEffect, useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom'
import Swal from 'sweetalert2'

export default function StoryDetails() {
    let noPreviewImage = 'data:image/jpeg;base64,'

    let { id } = useParams()
    let [storyFormValue, setStoryFormValue] = useState({
        storyName: '',
        storyDescription: '',
        storyStatus: 1
    })
    let apiBasePath = import.meta.env.VITE_APIBASEPATH

    let [storyImage, setStoryImage] = useState(noPreviewImage)
    let [bannerImage, setBannerImage] = useState(noPreviewImage)
    const storyimage = (event) => {
        const file = event.target.files[0];
        if (file) {
            setStoryImage(URL.createObjectURL(file));
        }
    };
    const bannerimage = (event) => {
        const file = event.target.files[0];
        if (file) {
            setBannerImage(URL.createObjectURL(file));
        }
    };


    let setValueGetValue = (event) => {
        let inputName = event.target.name
        let inputValue = event.target.value
        let oldData = { ...storyFormValue }
        oldData[inputName] = inputValue
        if (inputName == 'storyStatus') {
            oldData[inputName] = Number(inputValue)
        }
        setStoryFormValue(oldData)
    }

    let navigate=useNavigate()


    let saveStory = (event) => {
        event.preventDefault()
        let formValue = new FormData(event.target)
        if (id == undefined) {
            axios.post(`${apiBasePath}admin/story/add`, formValue)
                .then((res) => {
                    // console.log(res.data)
                    if (res.data.status) {
                        Swal.fire({
                            position: "top-end",
                            icon: "success",
                            title: res.data.msg,
                            showConfirmButton: false,
                            timer: 1500
                        });
                        setTimeout(() => {
                            navigate("/home/story/story-view")
                        }, 1500);

                    }
                    else {
                        Swal.fire({
                            title: "Oops!",
                            text: "Something Went Wrong !",
                            icon: "error"
                        });
                    }
                })
        }
        else{
            axios.put(`${apiBasePath}admin/story/update/${id}`, formValue)
            .then((res) => {
                if (res.data.status) {
                    Swal.fire({
                        position: "top-end",
                        icon: "success",
                        title: res.data.msg,
                        showConfirmButton: false,
                        timer: 1500
                    });
                    setTimeout(() => {
                        navigate("/home/story/story-view")
                    }, 1500);

                }
                else {
                    Swal.fire({
                        title: "Oops!",
                        text: "Something Went Wrong !",
                        icon: "error"
                    });
                }
            })
        }
    }


    useEffect(() => {
        
        if (id) {
            axios.get(`${apiBasePath}admin/story/edit/${id}`)
                .then((res) => res.data)
                .then((finalRes) => {
                    setStoryFormValue({
                        storyName:finalRes.data.storyName,
                        storyDescription:finalRes.data.storyDescription,
                        storyStatus:finalRes.data.storyStatus
                    })
                    setStoryImage(`${apiBasePath+finalRes.staticImagePath.storyImagePath+finalRes.data.storyImage}`)
                    setBannerImage(`${apiBasePath+finalRes.staticImagePath.storyBannerPath+finalRes.data.storyBanner}`)
                })

        }
        else{
            setStoryFormValue({
               storyName:'',
               storyDescription:'',
               storyStatus:1
            })
            setStoryImage(noPreviewImage) 
            setBannerImage(noPreviewImage)
        }
        
    }, [id])








    return (
        <div>
            <div className='py-3 px-4'>
                Home / Story / <span className='text-blue-700 font-semibold'>Story Details</span>
            </div> <hr />

            <div className='p-3'>
                <div className='border-[1px] border-gray-300 rounded-[10px]'>
                    <div className='p-2 text-[20px] font-semibold bg-gray-200 rounded-[10px_10px_0px_0px] border-[1px] border-b-gray-400'>
                        Our Story's
                    </div>
                    <div className='p-3'>
                        <form action="" onSubmit={saveStory}>
                            <div className='flex flex-col mb-4 gap-2'>
                                <label htmlFor="" className='font-semibold'>Story Name</label>
                                <input type="text" name='storyName' value={storyFormValue.storyName} onChange={setValueGetValue} className='border-[1px] border-gray-300 p-2 rounded-lg' placeholder=' Story Name' />
                            </div>
                            <div className='py-5 grid items-center gap-6 grid-cols-2'>
                                <div className='py-5 flex flex-col gap-2'>
                                    <label htmlFor="" className='font-semibold'>Image </label>
                                    <input type="file" name="storyImage" onChange={storyimage} className=" w-full border border-gray-200 shadow-sm rounded-lg text-sm  file:bg-gray-50 file:border-0 file:me-4  file:py-3 file:px-4 " multiple="" />
                                </div>
                                {storyImage && (
                                    <img
                                        src={storyImage}
                                        alt="Preview"
                                        className=" w-[100px] h-[100px] object-cover rounded-lg shadow-lg"
                                    />
                                )}
                            </div>

                            <div className='py-5 grid items-center gap-6 grid-cols-2'>
                                <div className='py-5 flex flex-col gap-2'>
                                    <label htmlFor="" className='font-semibold'>Banner Image</label>
                                    <input type="file" name="storyBanner" onChange={bannerimage} className=" w-full border border-gray-200 shadow-sm rounded-lg text-sm  file:bg-gray-50 file:border-0 file:me-4  file:py-3 file:px-4 " multiple="" />
                                </div>
                                {bannerImage && (
                                    <img
                                        src={bannerImage}
                                        alt="Preview"
                                        className=" w-[100px] h-[100px] object-cover rounded-lg shadow-lg"
                                    />
                                )}
                            </div>
                            <div className='mb-5 flex flex-col gap-2'>
                                <label htmlFor="" className='font-semibold'> Description</label>
                                <textarea name="storyDescription" value={storyFormValue.storyDescription} onChange={setValueGetValue} className='border-[1px] rounded-lg border-gray-300 resize-none p-2 h-[100px]' placeholder=' Description...' />
                            </div>


                            <div>
                                <p className='font-semibold mb-2' htmlFor="">Status :</p>
                                <div className='flex items-center gap-6'>
                                    <div className='flex items-center gap-2'>
                                        <input type="radio" value='1' checked={storyFormValue.storyStatus == 1} onChange={setValueGetValue} name='storyStatus' id='active' />
                                        <label htmlFor="active">Active</label>
                                    </div>
                                    <div className='flex items-center gap-2'>
                                        <input type="radio" value='0' checked={storyFormValue.storyStatus == 0} onChange={setValueGetValue} name='storyStatus' id='deactive' />
                                        <label htmlFor="deactive">Deactive</label>
                                    </div>
                                </div>
                            </div>

                            <div className='mt-7'>
                                <button type='submit' className='p-2 bg-purple-600 px-5 text-white font-semibold rounded-md'>
                                    {id ? 'Update Story' : 'Add Story'}
                                </button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    )
}
