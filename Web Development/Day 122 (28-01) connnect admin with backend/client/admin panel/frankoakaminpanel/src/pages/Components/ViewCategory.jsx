...
export default function ViewCategory() {
    ...
    function showData() {
        axios.get(`${apiBasePath}admin/category/view`)
            .then((res) => res.data)
            .then((finalRes) => {
    ...
    function singleDelete(id) {
        axios.delete(`${apiBasePath}admin/category/singleDelete/${id}`)
            .then((res) => res.data)
            .then((finalRes) => {
                Swal.fire({
                    title: "Good job!",
    ...
    function multiDelete() {
        if (allIds.length > 0) {
            ...
            axios.post(`${apiBasePath}admin/category/multiDelete`, obj)
                .then((res) => res.data)
                .then((finalRes) => {
                    Swal.fire({
                        title: "Good job!",
                        ...
                        <tbody>
                            {categoryData.length > 0 ?
                                    ...
                                    return (
                                        <tr className='border-b-gray-300  border-[1px]' key={index}>
                                            <td className='py-2 text-start ps-5'>
                                                <input onChange={getMultiDelete} type="checkbox" className='w-4 h-4' name='deletecheck' value={items._id} />
                                            </td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>{index + 1}</td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>{items.catName}</td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>
                                                <img src={apiBasePath + imageStaticPath + items.catImage} className='w-[70px] h-[70px] rounded-lg' alt="" />
                                            </td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>
                                                <p title={items.catDesc}>{items.catDesc}</p>
                                            </td>
                                            <td className='py-2 text-start ps-5 flex pt-7 items-center gap-2'>
                                                ...
                                                <Link to={`/home/parent-category/add-category/${items._id}`}>
                                            ...
                                            <td className='py-2 text-start ps-5 text-gray-500'>{(items.catStatus) ? "Active" : "Deactive"}</td>
                                        </tr>
                                    )
                                })
                                ...
                                <tr className='border-b-gray-300  border-[1px]'>
                                    <td colSpan={7} className='py-2 text-start ps-5'>No Data Found</td>
                                </tr>
                            }
