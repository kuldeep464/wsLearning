...
export default function StoryView() {
    ...
    function showData() {
        axios.get(`${apiBasePath}admin/story/view`)
            .then((res) => res.data)
            .then((finalRes) => {
                setStoryData(finalRes.data)
    ...
    function singleDelete(id) {
        axios.delete(`${apiBasePath}admin/story/singleDelete/${id}`)
            .then((res) => res.data)
            .then((finalRes) => {
                Swal.fire({
                    title: "Good job!",
    ...
    function multiDelete() {
        if (allIds.length > 0) {
            ...
            axios.post(`${apiBasePath}admin/story/multiDelete`, obj)
                .then((res) => res.data)
                .then((finalRes) => {
                    Swal.fire({
                        title: "Good job!",
    ...
    return (
        <div>
            ...
            <div className='py-3 px-4'>
                Home / Story / <span className='text-blue-700 font-semibold'>Story View</span>
            </div> <hr />
                    ...
                    <table className='w-[100%]'>
                        <thead className='w-[100%]'>
                            <tr className='font-semibold text-gray-700 text-[15px]'>
                                ...
                                <th className='py-2 text-start ps-5'>STORY NAME</th>
                                <th className='py-2 text-start ps-5'>IMAGE</th>
                                <th className='py-2 text-start ps-5'>BANNER</th>
                                    ...
                                    return (
                                        <tr className='border-b-gray-300  border-[1px]' key={index}>
                                            <td className='py-2 text-start ps-5'>
                                                <input type="checkbox" onChange={getMultiDelete} value={items._id} className='w-4 h-4' name='deletecheck' />
                                            </td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>{index + 1}</td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>{items.storyName}</td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>
                                                <img src={apiBasePath + imageStaticPath.storyImagePath + items.storyImage} className='w-[70px] h-[70px] rounded-lg' alt="" />
                                            </td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>
                                                <img src={apiBasePath + imageStaticPath.storyBannerPath + items.storyBanner} className='w-[100px] h-[70px] rounded-lg' alt="" />
                                            </td>
                                            <td className='py-2 text-start ps-5 text-gray-500'>
                                                <p title={items.storyDescription}>{items.storyDescription}</p>
                                            </td>
                                            ...
                                            <td className='py-2 text-start ps-5 flex pt-7 items-center gap-2'>
                                                ...
                                                <Link to={`/home/story/story-details/${items._id}`}>
                                            ...
                                            <td className='py-2 text-start ps-5 text-gray-500'>
                                                {(items.storyStatus) ? 'Active' : 'Deactive'}
                                            </td>
                                        </tr>
