...
export default function ViewSize() {
    ...
    function getSizeDetails() {
        axios.get(`${import.meta.env.VITE_APIBASEPATH}admin/size/view`)
            .then((res) => res.data)
            .then((finalRes) => {
    ...
    function singleDelete(id) {
        axios.delete(`${import.meta.env.VITE_APIBASEPATH}admin/size/singleDelete/${id}`)
            .then((res) => res.data)
            .then((finalRes) => {
                Swal.fire({
                    title: "Good job!",
    ...
    function multiDelete() {
        ...
        if (allIds.length > 0) {
            axios.post(`${import.meta.env.VITE_APIBASEPATH}admin/size/multiDelete`, obj)
                .then((res) => res.data)
                .then((finalRes) => {
                    Swal.fire({
                        title: "Good job!",
    ...
    return (
        <div>
            <div className='py-3 px-4'>
                Home / Size / <span className='text-blue-700 font-semibold'>View Size</span>
            </div> <hr />
                    ...
                    <table className='w-[100%]'>
                        <thead className='w-[100%]'>
                            <tr className='font-semibold text-gray-700 text-[15px]'>
                                ...
                                <th className='py-2 text-start ps-5'>SIZE NAME</th>
                        ...
                        <tbody>
                            ...
                            {sizeData.length > 0 ?
                                ...
                                sizeData.map((items, index) => {
                                    return (
                                        <tr className='border-b-gray-300  border-[1px]' key={index}>
                                            ...
                                            <td className='py-2 text-start ps-5 text-gray-500'>{items.sizeName}</td>
                                            <td className='py-2 text-start ps-5 flex items-center gap-2'>
                                                ...
                                                <Link to={`/home/size/size-details/${items._id}`}>
                                                    <button>
                                            ...
                                            <td className='py-2 text-start ps-5 text-gray-500'>{(items.sizeStatus) ? "Active" : "Deactive"}</td>
                                        </tr>
                                    )
                                })
                                ...
                                <tr className='border-b-gray-300  border-[1px]'>
                                    <td colSpan={4} className='py-2 text-start ps-5 flex items-center gap-2 text-gray-500'>No Size Found</td>
                                </tr>
                            }
