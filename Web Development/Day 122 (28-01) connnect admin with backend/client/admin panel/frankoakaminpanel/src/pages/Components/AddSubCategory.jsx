import axios from 'axios';
import React, { useEffect, useState } from 'react'
import { useNavigate, useParams } from 'react-router-dom';
import Swal from 'sweetalert2';

export default function AddSubCategory() {
    let noPreviewImage = 'data:image/jpeg;base64,'

    const [subCatImage, setSubCatImage] = useState(noPreviewImage);
    let [parentCat, setParentCat] = useState([])
    let { id } = useParams()
    let apiBasePath = import.meta.env.VITE_APIBASEPATH
    let navigate = useNavigate()
    let [subCatFormValue, setSubCatFormValue] = useState({
        subCategoryName: '',
        parentCategoryName: '',
        subCategoryDescription: '',
        status: 1
    })

    let getParentCategory = () => {
        axios.get(`${apiBasePath}admin/subCategory/parent-category`)
            .then((res) => res.data)
            .then((finalRes) => {
                setParentCat(finalRes.data)
            })
    }

    let saveSubCategory=(event)=>{
        event.preventDefault()
        let formValue = new FormData(event.target)
        if (id == undefined) {
            axios.post(`${apiBasePath}admin/subCategory/add`, formValue)
                .then((res) => {
                    console.log(res.data)
                    if (res.data.status) {
                        Swal.fire({
                            position: "top-end",
                            icon: "success",
                            title: res.data.msg,
                            showConfirmButton: false,
                            timer: 1500
                        });
                        setTimeout(() => {
                            navigate("/home/sub-category/view-sub-category")
                        }, 1500);

                    }
                    else {
                        Swal.fire({
                            title: "Oops!",
                            text: "Sub Category Name Already Exists",
                            icon: "error"
                        });
                    }
                })
        }
        else{
            axios.put(`${apiBasePath}admin/subCategory/update/${id}`, formValue)
            .then((res) => {
                if (res.data.status) {
                    Swal.fire({
                        position: "top-end",
                        icon: "success",
                        title: res.data.msg,
                        showConfirmButton: false,
                        timer: 1500
                    });
                    setTimeout(() => {
                        navigate("/home/sub-category/view-sub-category")
                    }, 1500);

                }
                else {
                    Swal.fire({
                        title: "Oops!",
                        text: "Sub Category Name Already Exists",
                        icon: "error"
                    });
                }
            })
        }
    }

    useEffect(() => {
        getParentCategory()
    }, [])

    const subcatimage = (event) => {
        const file = event.target.files[0];
        if (file) {
            setSubCatImage(URL.createObjectURL(file));
        }
    };

    let setValueGetValue = (event) => {
        let inputName = event.target.name
        let inputValue = event.target.value
        let oldData = { ...subCatFormValue }
        oldData[inputName] = inputValue
        if (inputName == 'status') {
            oldData[inputName] = Number(inputValue)
        }
        setSubCatFormValue(oldData)
    }

    useEffect(() => {
        
        if (id) {
            axios.get(`${apiBasePath}admin/subCategory/edit/${id}`)
                .then((res) => res.data)
                .then((finalRes) => {
                    setSubCatFormValue({
                        subCategoryName: finalRes.data.subCategoryName,
                        subCategoryDescription: finalRes.data.subCategoryDescription,
                        status: finalRes.data.status ? 1 : 0,
                        parentCategoryName: finalRes.data.parentCategoryName.catName
                    })
                    setSubCatImage(`${apiBasePath+finalRes.staticImagePath+finalRes.data.subCategoryImage}`)
                })

        }
        else{
            setSubCatFormValue({
                subCategoryName: '',
                parentCategoryName: '',
                subCategoryDescription: '',
                status: 1
            })
            setSubCatImage(noPreviewImage) 
        }
        
    }, [id])
    return (
        <div>
            <div className='py-3 px-4'>
                Home / Sub Category / <span className='text-blue-700 font-semibold'>Add Sub Category</span>
            </div> <hr />

            <div className='p-3'>
                <div className='border-[1px] border-gray-300 rounded-[10px]'>
                    <div className='p-2 text-[20px] font-semibold bg-gray-200 rounded-[10px_10px_0px_0px] border-[1px] border-b-gray-400'>
                        Add Sub Category
                    </div>
                    <div className='p-3'>
                        <form action="" onSubmit={saveSubCategory}>
                            <div className='flex flex-col mb-4 gap-2'>
                                <label htmlFor="" className='font-semibold'>Category Name</label>
                                <input type="text" name='subCategoryName' value={subCatFormValue.subCategoryName} onChange={setValueGetValue} className='border-[1px] border-gray-300 p-2 rounded-lg' placeholder='Enter Category Name' />
                            </div>
                            <div className='flex flex-col gap-2'>
                                <label htmlFor="" className='font-semibold'>Parent Category Name</label>
                                <select name='parentCategoryName' className='border-[1px] border-gray-300 p-2 rounded-lg'  >
                                    <option  selected={subCatFormValue.parentCategoryName==''} disabled>
                                        ---Select Parent Category---
                                    </option>

                                    {parentCat.map((items, index) => {
                                        return (
                                            <option value={items._id} key={index} selected={subCatFormValue.parentCategoryName==items.catName}>
                                                {items.catName}
                                            </option>
                                        )
                                    })}


                                </select>
                            </div>
                            <div className='py-5 grid items-center gap-6 grid-cols-2'>
                                <div className='py-5 flex flex-col gap-2'>
                                    <label htmlFor="" className='font-semibold'>Category Image</label>
                                    <input type="file" name="subCategoryImage" onChange={subcatimage} className=" w-full border border-gray-200 shadow-sm rounded-lg text-sm  file:bg-gray-50 file:border-0 file:me-4  file:py-3 file:px-4 " multiple="" />
                                </div>
                                {subCatImage && (
                                    <img
                                        src={subCatImage}
                                        alt="Preview"
                                        className=" w-[100px] h-[100px] object-cover rounded-lg shadow-lg"
                                    />
                                )}
                            </div>
                            <div className='mb-5 flex flex-col gap-2'>
                                <label htmlFor="" className='font-semibold'>Category Description</label>
                                <textarea name="subCategoryDescription" id="" value={subCatFormValue.subCategoryDescription} onChange={setValueGetValue} className='border-[1px] rounded-lg border-gray-300 resize-none p-2 h-[100px]' placeholder='Add Product Description...' />
                            </div>


                            <div>
                                <p className='font-semibold mb-2' htmlFor="">Status :</p>
                                <div className='flex items-center gap-6'>
                                    <div className='flex items-center gap-2'>
                                        <input type="radio" name='status' id='active' onChange={setValueGetValue} checked={subCatFormValue.status == 1} value='1' />
                                        <label htmlFor="active">Active</label>
                                    </div>
                                    <div className='flex items-center gap-2'>
                                        <input type="radio" name='status' id='deactive' onChange={setValueGetValue} checked={subCatFormValue.status == 0} value='0' />
                                        <label htmlFor="deactive">Deactive</label>
                                    </div>
                                </div>
                            </div>

                            <div className='mt-7'>
                                <button type='submit' className='p-2 bg-purple-600 px-5 text-white font-semibold rounded-md'>
                                    {id ? 'Update Sub Category' : 'Add Sub Category'}
                                </button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    )
}
