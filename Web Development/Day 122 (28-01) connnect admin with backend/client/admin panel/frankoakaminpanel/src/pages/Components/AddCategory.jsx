import React, { useEffect, useState } from 'react'
import axios from "axios"
import Swal from 'sweetalert2'
import { useNavigate, useParams } from 'react-router-dom';


export default function AddCategory() {
    let noPreviewImage = 'data:image/jpeg;base64,'
    const [parentcatImage, setParentcatImage] = useState(noPreviewImage);
    let [formDataValue, setformDataValue] = useState({
        categoryName: '',
        categoryDescription: '',
        status: 1
    })

    let { id } = useParams()
    const parentcategory = (event) => {
        const file = event.target.files[0];
        if (file) {
            setParentcatImage(URL.createObjectURL(file));
        }
        else {
            setParentcatImage(noPreviewImage)
        }
    };

    let navigate = useNavigate()
    let apiBasePath = import.meta.env.VITE_APIBASEPATH
    let addCategory = (event) => {



        event.preventDefault();
        let formValue = new FormData(event.target)

        if (id == undefined) {
            axios.post(`${apiBasePath}admin/category/add`, formValue)
                .then((res) => {
                    console.log(res.data)
                    if (res.data.status) {
                        Swal.fire({
                            position: "top-end",
                            icon: "success",
                            title: res.data.msg,
                            showConfirmButton: false,
                            timer: 1500
                        });
                        setTimeout(() => {
                            navigate("/home/parent-category/view-category")
                        }, 1500);

                    }
                    else {
                        Swal.fire({
                            title: "Oops!",
                            text: "Category Name Already Exists",
                            icon: "error"
                        });
                    }
                })
        }
        else{
            axios.put(`${apiBasePath}admin/category/update/${id}`, formValue)
            .then((res) => {
                if (res.data.status) {
                    Swal.fire({
                        position: "top-end",
                        icon: "success",
                        title: res.data.msg,
                        showConfirmButton: false,
                        timer: 1500
                    });
                    setTimeout(() => {
                        navigate("/home/parent-category/view-category")
                    }, 1500);

                }
                else {
                    Swal.fire({
                        title: "Oops!",
                        text: "Category Name Already Exists",
                        icon: "error"
                    });
                }
            })
        }
    }

    let getValueSetValue = (event) => {
        let inputName = event.target.name
        let inputValue = event.target.value
        let oldData = { ...formDataValue }
        oldData[inputName] = inputValue

        if (inputName == 'status') {
            oldData[inputName] = Number(inputValue)
        }

        setformDataValue(oldData)
    }
    useEffect(() => {
        if (id) {
            axios.get(`${apiBasePath}admin/category/edit/${id}`)
                .then((res) => res.data)
                .then((finalRes) => {
                    setformDataValue({
                        categoryName: finalRes.data.catName,
                        categoryDescription: finalRes.data.catDesc,
                        status: finalRes.data.catStatus ? 1 : 0
                    })
                    setParentcatImage(`${apiBasePath+finalRes.staticImagePath+finalRes.data.catImage}`)
                })

        }
        else{
            setformDataValue({
                categoryName: '',
                categoryDescription: '',
                status: 1
            })
            setParentcatImage(noPreviewImage) 
        }
    }, [id])
    return (
        <div>
            <div className='py-3 px-4'>
                Home / Parent Category / <span className='text-blue-700 font-semibold'>Add Category</span>
            </div> <hr />

            <div className='p-3'>
                <div className='border-[1px] border-gray-300 rounded-[10px]'>
                    <div className='p-2 text-[20px] font-semibold bg-gray-200 rounded-[10px_10px_0px_0px] border-[1px] border-b-gray-400'>
                        Add Category
                    </div>
                    <div className='p-3'>
                        <form action="" onSubmit={addCategory}>
                            <div className='flex flex-col gap-2'>
                                <label htmlFor="" className='font-semibold'>Category Name</label>
                                <input type="text" onChange={getValueSetValue} value={formDataValue.categoryName} name='categoryName' className='border-[1px] border-gray-300 p-2 rounded-lg' placeholder='Enter Category Name' />
                            </div>
                            <div className='py-5 grid items-center gap-28 grid-cols-2'>
                                <div className='py-5 flex flex-col gap-2'>
                                    <label htmlFor="" className='font-semibold'>Category Image</label>
                                    <input type="file" name="categoryImage" onChange={parentcategory} className=" w-full border border-gray-200 shadow-sm rounded-lg text-sm  file:bg-gray-50 file:border-0 file:me-4  file:py-3 file:px-4 " multiple="" />
                                </div>
                                {parentcatImage && (
                                    <img
                                        src={parentcatImage}
                                        alt="Preview"
                                        className=" w-[80px] h-[70px] object-cover rounded-lg shadow-lg mt-6"
                                    />
                                )}
                            </div>
                            <div className='mb-5 flex flex-col gap-2'>
                                <label htmlFor="" className='font-semibold'>Category Description</label>
                                <textarea onChange={getValueSetValue} name="categoryDescription" value={formDataValue.categoryDescription} id="" className='border-[1px] rounded-lg border-gray-300 resize-none p-2 h-[100px]' placeholder='Add Product Description...' />
                            </div>


                            <div>
                                <p className='font-semibold mb-2' htmlFor="">Status :</p>
                                <div className='flex items-center gap-6'>
                                    <div className='flex items-center gap-2'>
                                        <input type="radio" onChange={getValueSetValue} name='status' value='1' checked={formDataValue.status == 1} id='active' />
                                        <label htmlFor="active">Active</label>
                                    </div>
                                    <div className='flex items-center gap-2'>
                                        <input type="radio" onChange={getValueSetValue} name='status' checked={formDataValue.status == 0} value='0' id='deactive' />
                                        <label htmlFor="deactive">Deactive</label>
                                    </div>
                                </div>
                            </div>

                            <div className='mt-7'>
                                <button type='submit' className='p-2 hover:bg-red-600 duration-300 bg-purple-600 px-5 text-white font-semibold rounded-md'>
                                    {id ? 'Update Category' : 'Add Category'}
                                </button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    )
}
