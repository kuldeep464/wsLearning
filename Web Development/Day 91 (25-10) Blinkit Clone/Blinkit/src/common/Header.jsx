...
export default function Header() {
    return (
        <header className='py-4 grid grid-flow-row lg:grid-flow-col gap-3 lg:grid-cols-[10%_75%_auto] xl:grid-cols-[10%_80%_auto] items-center px-3'>
            ...
            <div className='flex justify-center flex-col sm:flex-row items-center gap-[20px] px-5'>
                <div className='text-center'>
                    <h2 className='font-bold text-[18px]'>Delivery in 10 minutes</h2>
                    <div>

                        <button className='flex items-center'><span className='text-[15px]'>Gurugram, Haryana, India</span> <IoMdArrowDropdown className='text-[25px]' /></button>

                    </div>
                </div>
                <div className='flex items-center gap-2 bg-gray-200 px-3 py-2 rounded-[8px] w-[100%] xl:w-[70%] lg:w-[60%] sm:w-[50%]'>
                    <CiSearch className='text-[20px]' />
                    <input type="text" placeholder='search "products"' className=' bg-gray-200 focus:outline-none' />
                </div>
                <div>
                    <button className='text-[20px] px-4 text-gray-700'>Login</button>
                </div>
            </div>
            <div className='flex justify-end'>
                <button className='bg-[rgb(12,131,31)] text-white p-3 font-semibold flex gap-1 rounded-[7px]'><IoCartOutline className='text-[25px] font-bold'/>My Cart</button>
            </div>
        </header>
    )
}
